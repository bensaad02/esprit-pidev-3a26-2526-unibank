...
class LoginSuccessHandler implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token): RedirectResponse
    {
        ...
        if (!$user->isEstActif()) {
            ...
            if ($user->getBanUntil() !== null && $user->getBanUntil() <= new \DateTime()) {
                $user->setEstActif(true);
                $user->setBanUntil(null);
                $user->setBanReason(null);
                $this->em->flush();
                // allow login to continue normally below
            } else {
                $this->security->logout(false);
                $until = $user->getBanUntil();
                $msg = $until
                    ? 'Votre compte est banni jusqu\'au ' . $until->format('d/m/Y') . '.'
                    : 'Votre compte a été banni définitivement. Contactez le support.';
