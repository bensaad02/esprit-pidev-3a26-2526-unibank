...
// Entite credit utilisee dans certains emails metier.
use App\Entity\Credit;
// Entite utilisateur utilisee comme destinataire des emails.
use App\Entity\Utilisateur;
// Service Symfony Mailer pour envoyer les emails.
use Symfony\Component\Mailer\MailerInterface;
// Represente une adresse email formatee avec nom + email.
use Symfony\Component\Mime\Address;
// Represente le message email a envoyer.
use Symfony\Component\Mime\Email;

class EmailService
{
    public function __construct(
        // Mailer injecte par Symfony pour envoyer les emails.
    ...
    // Construit l'adresse de l'expediteur a reutiliser dans tous les emails.
    private function from(): Address
    {
        // Retourne une adresse complete avec email et nom d'affichage.
...
HTML;
    ...
    {
        // Recupere le nom complet du client pour l'afficher dans l'email.
        $name = $user->getPrenom() . ' ' . $user->getNom();
        // Formate le montant du credit pour un affichage lisible.
        $montant = number_format((float) $credit->getMontant(), 2, ',', ' ');
        // Formate la mensualite du credit.
        $mensualite = number_format((float) $credit->getMensualite(), 2, ',', ' ');
        // Recupere la duree du credit en mois.
        $duree = (int) $credit->getDureeEnMois();

        // Construit le contenu HTML de l'email d'approbation.
        $html = <<<HTML
<!DOCTYPE html>
<html>
<head>
...
HTML;
        ...
        // Cree l'objet email avec expediteur, destinataire, sujet et contenu HTML.
        $email = (new Email())
            ->from($this->from())
            ->to($user->getEmail())
        ...
        // Envoie l'email au client via Symfony Mailer.
        $this->mailer->send($email);
    }
}
