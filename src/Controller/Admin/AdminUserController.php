...
class AdminUserController extends AbstractController
{
    ...
    public function ban(int $id, Request $request, EntityManagerInterface $em, EmailService $emailService): Response
    {
        $user = $em->getRepository(Utilisateur::class)->find($id);
        if (!$user) {
            $this->addFlash('error', 'Utilisateur introuvable.');
        ...
        if (!$user->isEstActif()) {
            ...
            $em->flush();
            try { $emailService->sendUnbanEmail($user); } catch (\Exception) {}
            $this->addFlash('success', $user->getPrenom() . ' ' . $user->getNom() . ' a été débanni.');
            return $this->redirectToRoute('app_admin_users');
        }

        // Ban with duration
        $duration = $request->request->get('duration', 'permanent');
        $reason   = trim($request->request->get('reason', '')) ?: null;

        $banUntil = match($duration) {
            '1d'  => (new \DateTime())->modify('+1 day'),
            '3d'  => (new \DateTime())->modify('+3 days'),
            '1w'  => (new \DateTime())->modify('+1 week'),
            '2w'  => (new \DateTime())->modify('+2 weeks'),
            '1m'  => (new \DateTime())->modify('+1 month'),
            '3m'  => (new \DateTime())->modify('+3 months'),
            '6m'  => (new \DateTime())->modify('+6 months'),
            '1y'  => (new \DateTime())->modify('+1 year'),
        ...
        try { $emailService->sendBanEmail($user); } catch (\Exception) {}

        $label = $banUntil ? 'jusqu\'au ' . $banUntil->format('d/m/Y') : 'définitivement';
        $this->addFlash('success', $user->getPrenom() . ' ' . $user->getNom() . ' banni ' . $label . '.');
        return $this->redirectToRoute('app_admin_users');
    }
