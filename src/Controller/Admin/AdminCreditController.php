...
class AdminCreditController extends AbstractController
{
    ...
    public function finalize(int $id, EntityManagerInterface $em, ContractPdfGenerator $pdfGen, SupabaseStorageService $supabase, ValidatorInterface $validator, ContractBrevoService $contractBrevoService, LoggerInterface $logger): Response
    {
        ...
        $publicUrl = $supabase->upload($path, $pdfBytes);
        if ($publicUrl) {
            $contrat->setCheminFichier($publicUrl);
            $em->flush();
        }

        $emailSent = false;
        $brevoConfigured = $contractBrevoService->isConfigured();
    ...
    private function buildStatsData(CreditRepository $repo): array
    {
        $monthConfig = [
        ...
        foreach ($repo->findBy([], ['dateCreation' => 'DESC']) as $credit) {
            $dateCreation = $credit->getDateCreation();
            if (!$dateCreation) {
                continue;
            }

            $key = $dateCreation->format('Y-m');
            if (isset($months[$key])) {
                $months[$key]['count']++;
            }
        }
