...
class CreditController extends BaseClientController
{
    ...
    public function index(CreditRepository $repo, ContratRepository $contratRepo, ExchangeRateService $fx): Response
    {
        ...
        $contratsRaw = $contratRepo->findByClient($user->getIdUtilisateur());
        // Map contrat by credit ID for easy template access
        $contratsByCreditId = [];
        foreach ($contratsRaw as $contrat) {
            $contratsByCreditId[$contrat->getCredit()->getIdCredit()] = $contrat;
        }

        // Envoie les donnees au template Twig
        return $this->render('client/credit/index.html.twig', [
            'credits'            => $credits,
            'contrats'           => $contratsRaw,
            'contratsByCreditId' => $contratsByCreditId,
