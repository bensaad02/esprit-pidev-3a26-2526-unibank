...
class TransactionPdfController extends BaseClientController
{
    ...
    public function extrait(Request $request, EntityManagerInterface $em, TransactionBancaireRepository $repo, SupabaseStorageService $supabase): Response
    {
        ...
        $pdfBytes = $this->renderPdf($html, 'A4', 'portrait');

        $path = $supabase->makePath('extraits', 'Extrait_' . $compte->getNumeroCompte());
        ...
        // Fallback: direct download if Supabase fails
        return new Response($pdfBytes, 200, [
            'Content-Type'        => 'application/pdf',
            'Content-Disposition' => 'attachment; filename="Extrait_' . $compte->getNumeroCompte() . '_' . date('Ymd') . '.pdf"',
        ]);
    }

    #[Route('/receipt/{id}', name: 'app_client_receipt')]
    public function receipt(int $id, EntityManagerInterface $em, SupabaseStorageService $supabase): Response
    {
        $redirect = $this->checkApproved();
        if ($redirect) return $redirect;
        ...
        $pdfBytes = $this->renderPdf($html, [0, 0, 419.53, 595.28], 'portrait');

        $path = $supabase->makePath('receipts', 'Recu_TXN_' . $id);
        $url  = $supabase->upload($path, $pdfBytes);
        ...
        return new Response($pdfBytes, 200, [
            'Content-Type'        => 'application/pdf',
            'Content-Disposition' => 'attachment; filename="Recu_TXN_' . $id . '.pdf"',
        ]);
    }

    private function renderPdf(string $html, string|array $paper, string $orientation): string
    {
        $options = new Options();
